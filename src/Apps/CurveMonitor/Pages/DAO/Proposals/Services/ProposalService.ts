...
const parseProposal = (
  ...
  const quorum = bigNumToNumber(BigInt(x.minAcceptQuorum), 18);
  const support = bigNumToNumber(BigInt(x.supportRequired), 18);
  const votes = parseInt(x.voteCount, 10);
  const votesFor = bigNumToNumber(BigInt(x.votesFor), 18);
  const votesAgainst = bigNumToNumber(BigInt(x.votesAgainst), 18);
  const executed = x.executed;
  const totalSupply = bigNumToNumber(BigInt(x.totalSupply), 18);
